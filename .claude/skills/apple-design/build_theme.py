...
def build(brand_hex, name):
    ...
    L += [
        ...
        "  /* The kit hard-codes #FFFFFF on a filled button, which is right",
        "     for Apple's own palette and wrong for a light brand: white on",
        "     systemYellow is 1.25:1. This carries the label colour the",
        "     contrast pass actually chose. */",
        f"  --ios-on-accent: var(--{name}-on-accent);",
    ...
    values = {
        "accent":        (rgb_to_hex(accent_l), rgb_to_hex(accent_d)),
        "accentText":    (rgb_to_hex(text_l), rgb_to_hex(text_d)),
        "onAccent":      (rgb_to_hex(on_accent), rgb_to_hex(on_accent_d)),
        "label":         ("#1A1A1A", "#F5F5F5"),
        "background":    (BG_LIGHT, BG_DARK),
        "backgroundCard": (CARD_LIGHT, CARD_DARK),
        "accentIncreasedContrast": (rgb_to_hex(ic_fill_l), rgb_to_hex(ic_fill_d)),
        "accentTextIncreasedContrast": (rgb_to_hex(ic_l), rgb_to_hex(ic_d)),
    }
    return "\n".join(L), notes, values
...
# --- other platforms -------------------------------------------------
#
# --kind cross used to accept the flag and emit a web page, which is a
# promise of native support and the delivery of a prototype. The contrast
# pass has already resolved every value; the only thing missing was
# somewhere to put them. These follow the conventions of the exports in
# apple-ui-kit/tokens, including the caution each platform needs.

_SWIFT_HEAD = """// {name} brand tokens. Generated by build_theme.py -- do not hand-edit.
//
// Derived from {brand} by the same contrast pass that produced theme.css,
// so the web build and this agree by construction rather than by someone
// copying hex codes between them.
//
// Prefer the semantic APIs (Color.accentColor, .primary) wherever they
// serve: they follow Dark Mode and Increase Contrast on their own. Reach
// for these where the semantic API cannot -- custom drawing, a canvas,
// or a brand colour the system has no name for.

import SwiftUI

public enum {enum}Color {{"""

_KOTLIN_HEAD = """// {name} brand tokens. Generated by build_theme.py -- do not hand-edit.
//
// Derived from {brand} by the same contrast pass that produced theme.css.
//
// This is a brand palette on a platform with its own colour roles.
// Material expects its own scheme and its own controls, and users expect
// them too; carry the brand where brand consistency is the deliberate
// goal, not by default.
...
def emit_swift(values, name, brand):
    out = [_SWIFT_HEAD.format(name=name, brand=brand, enum=_camel(name))]
    for key, (light, dark) in values.items():
        out.append(f"    /// light {light} - dark {dark}")
        out.append(f"    public static let {key} = Color(uiColor: UIColor {{ t in")
        out.append(f"        t.userInterfaceStyle == .dark")
        out.append(f"            ? UIColor(red: {_c(dark, 0)}, green: {_c(dark, 1)}, "
                   f"blue: {_c(dark, 2)}, alpha: 1.000)")
        out.append(f"            : UIColor(red: {_c(light, 0)}, green: {_c(light, 1)}, "
                   f"blue: {_c(light, 2)}, alpha: 1.000)")
        out.append("    })")
    out.append("}")
    return "\n".join(out) + "\n"


def emit_kotlin(values, name, brand):
    pkg = re.sub(r"[^a-z0-9]", "", name.lower()) or "brand"
    out = [_KOTLIN_HEAD.format(name=name, brand=brand, enum=_camel(name), pkg=pkg)]
    for key, (light, dark) in values.items():
        out.append(f"    // light {light} - dark {dark}")
        out.append(f"    val {key} = Color(0xFF{light.lstrip('#')})")
        out.append(f"    val {key}Dark = Color(0xFF{dark.lstrip('#')})")
    out.append("}")
    return "\n".join(out) + "\n"


def emit_xml(values, name, brand):
    out = ['<?xml version="1.0" encoding="utf-8"?>',
           "<!--", f"    {name} brand tokens from {brand}.",
           "    Generated by build_theme.py -- do not hand-edit.", "-->",
           "<resources>"]
    for key, (light, dark) in values.items():
        snake = re.sub(r"(?<!^)(?=[A-Z])", "_", key).lower()
        out.append(f'    <color name="{snake}">{light}</color>')
        out.append(f'    <color name="{snake}_dark">{dark}</color>')
    out.append("</resources>")
    return "\n".join(out) + "\n"


def _c(hex_colour, i):
    return f"{hex_to_rgb(hex_colour)[i] / 255:.3f}"


FORMATS = {"css": None, "swift": emit_swift, "kotlin": emit_kotlin,
           "xml": emit_xml}
...
def main():
    ...
    ap.add_argument("--format", default="css", choices=sorted(FORMATS),
                    help="css (default), or a native token export")
    args = ap.parse_args()

    try:
        css, notes, values = build(args.colour, args.name)
    except ValueError as e:
        sys.exit(str(e))

    emitter = FORMATS.get(args.format)
    text = css if emitter is None else emitter(values, args.name, args.colour)
