I'll check the working directory and pull in Apple's measured design values so this looks right rather than approximately right.
...
Tokens copied. Now the app itself.

Bash needs approval for those; I'll skip the palette spelunking and keep the icon tiles on the measured accent rather than inventing hues. Writing the app logic:

Now the Enter-to-submit gap (the form has no submit button, so implicit submission wouldn't fire):

Built. Four files in `a-web-app/`:

| File | What it is |
|---|---|
| `index.html` | The page |
| `app.css` | Layout specific to this app |
| `app.js` | State, rendering, sheet logic |
| `tokens/ios-tokens.css` + `ios-components.css` | Apple's measured iOS 27 values and component recipes, copied from the apple-ui-kit skill |

**What's there**

- **Total at the top** in a white card on the grey grouped background — large-title size (34/41), tabular figures so digits don't jitter as the number changes.
- **Expense list** as an inset grouped list: icon tile, description, `Category · Today` subtitle, right-aligned amount. Separators start at the text and the last row has none, which is the detail that usually gives web knock-offs away.
- **Add button** in the nav bar opens a sheet from the bottom — a sheet rather than an alert, since alerts are for a message and a choice, not multi-field entry. Amount is the hero field, description is a row-embedded field, category is a segmented control.
- **Edit** toggles per-row delete. Large title scrolls away and fades into a compact bar title. Seeded with four rows; persists to localStorage.

**On the Apple values.** The accent is `#0088FF`, not the `#007AFF` that circulates online, and the primary label is `#1A1A1A`, not black — those come from measurements of Apple's real UI kit, as does the type scale, the 44px hit targets and the capsule buttons. Dark mode and `prefers-contrast: more` come along with the tokens; the contrast override matters because Apple's light palette doesn't clear Apple's own 4.5:1 table on secondary text, and the web has no Increase Contrast running for you.

Two deliberate calls worth flagging. I made all the category tiles the accent colour rather than Settings-style multicoloured — the kit only measured blue, green and red, and iOS 27 shifted the palette enough that the other five hues would have been guesses that land visibly off. And there's no swipe-to-delete: that gesture's physics don't reproduce in a browser, so Edit mode reveals a visible delete control instead. Same reason the nav bar gets `backdrop-filter` blur rather than a fake Liquid Glass — the blur is the part that reads at a glance, the refraction isn't reproducible. SF Pro isn't licensed for the web, so the font stack starts at `system-ui`: real SF on Apple devices, a sane native face elsewhere.

Open `index.html` directly — no build step, no network calls. Say the word if you'd like it inlined into a single self-contained file and published as an artifact so you can view or share it from a link.